 ...
 * @date   06.07.12
 */
class SimpleDateTimeBehavior extends CActiveRecordBehavior
{
    ...
    public function beforeSave(){
        ...
        foreach($this->getOwner()->metadata->tableSchema->columns as $columnName => $column){
            ...
            if ($column->dbType == 'date'){
                $this->getOwner()->{$columnName}=date('Y-m-d', strtotime(str_replace(",", "", $this->getOwner()->{$columnName})));
            }elseif ($column->dbType == 'datetime'){
                $this->getOwner()->{$columnName} = date('Y-m-d H:i:s', strtotime(str_replace(",", "", $this->getOwner()->{$columnName})));
            }
        }
    }
